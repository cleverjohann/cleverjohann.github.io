  ...
  const e = document.querySelectorAll("section"),
    ...
    n = (r) => {
      r.forEach((a) => {
        a.isIntersecting &&
          t.forEach((l) => {
            l.getAttribute("aria-label") == a.target.id
              ? l.classList.add("text-yellow-500")
...
async function ie(e, t, n, o, r, a, l, f) {
  ...
  return (
    document.dispatchEvent(i) &&
      (await i.loader(),
      i.defaultPrevented ||
        (ne(Z), i.navigationType !== "traverse" && k({ scrollX, scrollY }))),
    i
  );
}
...
const j = (e, t, n, o, r) => {
  ...
  if (
    ...
    if (e.hash) {
      ...
      const i = history.state;
      (location.href = e.href), history.state || E(i, "");
...
async function K(e, t, n, o, r) {
  ...
  async function f(i) {
    ...
    if (
      ...
      (M ??= new DOMParser()),
      (i.newDocument = M.parseFromString(s.html, s.mediaType)),
  ...
  if (((A = !1), P))
    p = document.startViewTransition(async () => await H(l, o, r));
  else {
    const i = (async () => {
      await new Promise((m) => setTimeout(m)), await H(l, o, r, Y());
    })();
    p = {
...
function pe() {
  ...
  O(() => {
    for (const t of document.getElementsByTagName("a"))
      S.has(t) || (v(t, "viewport") && (S.add(t), (e ??= ge()), e.observe(t)));
  });
}
function ge() {
  const e = new WeakMap();
